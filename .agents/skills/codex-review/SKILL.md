...
Never fall back to reviewing Codex's own output if Codex fails. If Codex can't run, say so and stop.
...
- **`plan`**: if there is an active in-conversation plan, write it to a temp file as above. Otherwise pick the newest file under `./.Codex/plans/` (`ls -t ./.Codex/plans/*.md 2>/dev/null | head -1`). If neither exists, report "no plan found" and stop.
